...
export function useLocalPairScanner(config: Partial<ScannerConfig> = {}) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const cacheKey = `scanner-spot-${scannerConfig.lookbackDays}`;
      try {
        ...
        if (cached) {
          ...
          if (age < TTL) {
            console.log(`[Scanner Cache] Using cached spot data (age: ${Math.round(age / 1000)}s)`);
      ...
      try {
        ...
        try {
          ...
          console.log(`[Scanner Cache] Saved spot data to cache`);
        } catch (e) {
          console.warn("[Scanner Cache] Failed to save cache:", e);
        }
